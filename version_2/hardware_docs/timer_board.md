...
24. `INT-TIMER-CONFIG-TIMEBASE-AND-ENABLE` - Bits 0 - 3 address every even-indexed bit of the main 36-bit divider up to bit 30, so 0x0 -> 1 MHz, 0x1 -> 250 KHz and so on. Bit 4 sets whether the timer is allowed to cause interrupts. Bit 5 sets whether the counter is currently counting. Bit 0 in this case is the actual 1 MHz signal.

## To bus (TX)

9. `MAIN-TIMER` - 8-bit section of the 36 bit main timer. Addressed by the `INT-AND-MAIN-TIMER-ADDRESS` (see above). Bits indexed are as follows based on that address: 0 -> 4-11, 1 -> 12-19, 2 -> 20-27, 3 -> 28-35. Bit 0 is actually 500 KHz because its still divided by 2.
